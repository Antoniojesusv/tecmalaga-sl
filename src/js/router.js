function Router(features = {}) {
  ...
  if (validationSpa.isValidFeaturesRouter(features))
...
Router.prototype = {
  ...
  goToRoute: function(htmlName) {
    (function(scope) {
      ...
      const HtmlAndJs = scope.contentToDownload(htmlName).map(element =>
        scope.dynamicCalls({
          scope,
          htmlName,
          element,
      ...
      Promise.all(HtmlAndJs).then(([html, js]) => {
        scope.putHtml(html);
        scope.putScript(js);
      });
    })(this);
